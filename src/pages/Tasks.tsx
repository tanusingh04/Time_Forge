      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-card rounded-xl p-6 border border-border space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <Input placeholder="Task title (e.g., Revise Calculus Ch.3)" value={title} onChange={(e) => setTitle(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleAdd()}/>
            <div className="space-y-2">
              {syllabusSubjects.length > 0 && !useCustomSubject ? (<>
                  <Select value={subject === "__custom__" ? "" : subject} onValueChange={(value) => {
                if (value === "__custom__") {
                  ...
                  {subject && subject !== "__custom__" && (<p className="text-xs text-muted-foreground">
                      Selected: <span className="font-medium">{subject}</span>
                    </p>)}
                </>) : (<>
                  <Input placeholder="Subject (e.g., Mathematics)" value={customSubject} onChange={(e) => setCustomSubject(e.target.value)}/>
        ...
        <div className="space-y-2">
          <AnimatePresence>
            {tasks.length === 0 && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12 text-muted-foreground">
                <AlertTriangle size={32} className="mx-auto mb-2 opacity-50"/>
                <p className="text-sm">No tasks yet. Add your first task above!</p>
              </motion.div>)}
            {tasks.map((task, i) => (<motion.div key={task.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ delay: i * 0.05 }} className={`flex items-center gap-4 p-4 rounded-xl border bg-card ${task.completed ? "opacity-60" : ""}`}>
                ...
                {!task.completed && (<Button onClick={() => toggleTask(task.id)} size="sm" className="bg-primary text-primary-foreground hover:bg-primary/90">
                    <CheckCircle2 size={16} className="mr-1.5"/>
                    Mark Done
                  </Button>)}
                {task.completed && (<Button onClick={() => toggleTask(task.id)} variant="outline" size="sm" className="text-muted-foreground">
