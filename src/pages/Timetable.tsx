...
const TimetablePage = () => {
    const { timetable, generateTimetable, tasks, savedTimetables, saveCurrentTimetable, loadTimetable, deleteSavedTimetable, } = useAppState();
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="flex justify-between items-start flex-wrap gap-4">
          ...
          <div className="flex gap-2">
            {timetable.length > 0 && (<Dialog open={saveDialogOpen} onOpenChange={setSaveDialogOpen}>
                ...
                <DialogContent>
                  ...
                  <div className="space-y-4">
                    <Input placeholder="Timetable name (optional)" value={timetableName} onChange={(e) => setTimetableName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSave()}/>
            ...
            <Button onClick={generateTimetable} className="bg-primary text-primary-foreground hover:bg-primary/90">
        ...
        {savedTimetables.length > 0 && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-card rounded-xl p-4 border border-border">
            <div className="flex items-center gap-2 mb-3">
              <History size={18} className="text-primary"/>
              <h2 className="font-semibold text-sm">Saved Timetables</h2>
            </div>
            <div className="flex flex-wrap gap-2">
              {savedTimetables.map((saved) => (<div key={saved.id} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-secondary border border-border">
                  <button onClick={() => loadTimetable(saved.id)} className="text-sm hover:text-primary transition-colors">
                    {saved.name}
                  </button>
                  <button onClick={() => deleteSavedTimetable(saved.id)} className="text-muted-foreground hover:text-destructive transition-colors" aria-label="Delete timetable">
                    <Trash2 size={14}/>
                  </button>
                </div>))}
            </div>
        ...
        {tasks.length === 0 && timetable.length === 0 && (<div className="text-center py-16 text-muted-foreground">
        ...
        {timetable.length > 0 && (<div className="space-y-2">
            {timetable.map((entry, i) => (<motion.div key={entry.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }} className={`flex items-center gap-4 p-4 rounded-xl border border-l-4 bg-card text-card-foreground ${typeStyles[entry.type] || ""}`}>
