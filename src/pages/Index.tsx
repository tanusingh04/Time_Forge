...
const Dashboard = () => {
    const { profile, tasks, timetable } = useAppState();
    ...
    const stats = [
        ...
        { icon: Clock, label: "Pending Tasks", value: (totalTasks - completedToday).toString(), color: "text-accent" },
    ];
    return (<Layout>
      <div className="max-w-5xl mx-auto space-y-8">
        {/* Hero Greeting */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-primary rounded-2xl p-8 text-primary-foreground shadow-glow-purple">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {stats.map((stat, i) => (<motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="bg-card rounded-xl p-5 border border-border shadow-sm">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {/* Quick Actions */}
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }} className="bg-card rounded-xl p-6 border border-border">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }} className="bg-card rounded-xl p-6 border border-border">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-semibold">Today's Schedule</h2>
              <Link to="/timetable" className="text-xs text-primary hover:underline">View all →</Link>
            </div>
            {upcomingEntries.length > 0 ? (<div className="space-y-3">
                ...
                return (<div key={entry.id} className={`p-3 rounded-lg border ${typeColors[entry.type] || "bg-card"}`}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="bg-card rounded-xl p-6 border border-border">
          ...
          {profile.goals && profile.goals.length > 0 ? (<ul className="grid sm:grid-cols-2 gap-4">
              {profile.goals.map((goal, index) => (<li key={index} className="flex items-start gap-3 p-4 rounded-lg bg-secondary/50 border border-border/50">
            ...
            </ul>) : (<div className="text-center py-8 bg-secondary/40 rounded-xl border border-dashed border-border">
